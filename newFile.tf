terraform {
  ...
  backend "s3" {
    bucket = "remote-state-do-not-delete"
    key = "remoteState/file_1.tf1"
    region = "us-east-1"
  }
}
...
resource "aws_db_snapshot" "test2" {
  db_instance_identifier = aws_db_instance.dbInstance2.id
  db_snapshot_identifier = "dbInstance1SnapShot"
}
...
module "automationGuarddutyDetector" {
  source = "./modules"
  providers = {
    aws = aws.ap-south-1
  }
